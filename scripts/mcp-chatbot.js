...
async function main() {
  ...
  // Connect to both MCP servers over HTTP.
  // Start the legislation server on port 3000 and the help server on port 3001
  // before running the chatbot (see CHATBOT.md for instructions).
  const legislationUrl = process.env.LEGISLATION_MCP_URL || "http://localhost:3000/mcp";
  const helpUrl = process.env.HELP_MCP_URL || "http://localhost:3001/mcp";

  const legislationTransport = new StreamableHTTPClientTransport(new URL(legislationUrl));
  const helpTransport = new StreamableHTTPClientTransport(new URL(helpUrl));

  const legislationClient = new Client(
    { name: "mcp-chatbot-legislation", version: "0.1.0" },
    { capabilities: {} }
  );
  const helpClient = new Client(
    { name: "mcp-chatbot-help", version: "0.1.0" },
    { capabilities: {} }
  );
  ...
  for (const tool of legislationTools) {
    toolClientMap.set(tool.name, legislationClient);
  }
  for (const tool of helpTools) {
    toolClientMap.set(tool.name, helpClient);
  }

  const allTools = [...legislationTools, ...helpTools];

  // Resources are provided by the legislation MCP server.
  const { resources } = await legislationClient.listResources();
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (parsed?.type === "tool" && parsed?.content?.name) {
        ...
        // Route the tool call to the server that registered it.
        const toolClient = toolClientMap.get(toolName);
        if (!toolClient) {
          console.warn(`Warning: unknown tool "${toolName}" — routing to legislation server by default.\n`);
        }
        const resolvedClient = toolClient || legislationClient;
        console.log(`Tool call: ${toolName}`);
        console.log("Arguments:");
        console.log(`${formatToolArgs(toolArgs)}\n`);
        const toolResult = await resolvedClient.callTool({
          name: toolName,
          arguments: toolArgs,
        });
      ...
      if (parsed?.type === "resource" && parsed?.content?.uri) {
        ...
        // Resources are served by the legislation MCP server.
        const resourceResult = await legislationClient.readResource({ uri: resourceUri });
  ...
  rl.close();
  await legislationClient.close();
  await helpClient.close();
}
